 ...
 *
 * @warning
 * 1. Компонент использует фиксированные размеры (25px для кубов), что может не адаптироваться ко всем размерам экрана
 * 2. z-index 9999 может конфликтовать с другими элементами с высоким z-index
 * 3. Полноэкранное позиционирование блокирует взаимодействие с подложкой
 * 4. Цвета кубов захардкожены (#3498db и #2ecc71)
 * 5. Анимация многоточия использует псевдоэлемент ::after
 * 6. На мобильных устройствах меняется только размер шрифта, но не размеры кубов
 *
 * @accessibility
 ...
 *
 * @todo
 * - Добавить пропсы для кастомизации текста сообщения
 * - Реализовать возможность изменения цветовой схемы кубов
 * - Добавить пропс для размера (small, medium, large)
 * - Реализовать вариант без полноэкранного покрытия (inline loader)
 * - Добавить aria-label и role для лучшей доступности
 * - Реализовать анимацию входа/выхода (fade in/out)
 * - Добавить пропс для отключения анимации (prefers-reduced-motion)
 * - Поддержка темной темы через CSS-переменные
 * - Улучшить адаптивность размеров кубов на мобильных
 * - Реализовать прогресс-бар или процент завершения
 * - Добавить возможность прерывания загрузки (кнопка отмены)
 *
 * @design
 ...
 *
 * @performance
 * CSS-анимации обычно хорошо оптимизированы браузерами, но:
 * - 9 одновременно анимируемых элементов могут повлиять на производительность на слабых устройствах
 * - Следует учитывать prefers-reduced-motion для пользователей с чувствительностью к анимации
 * - Можно рассмотреть использование transform и opacity для лучшей производительности
 */
